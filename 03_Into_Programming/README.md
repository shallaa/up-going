...
보통 이런 식 문은 프로그램 실행에 아무런 영향을 미치지 않기 때문에  유용하지 않습니다. `b`의 값에 접근하여 `2`를 곱한 다음 결과값을 가지고 아무 것도하지 않습니다.

보다 일반적인 식 문은 전체 표현식이 함수 호출 표현식 문( "Function" 참고 )이므로 호출 표현 문 ( "함수"참조)입니다.

보다 일반적인 식문은 스스로를 호출하여 식을 요청하는 호출 식 문 입니다. ( "Function" 참고 )
